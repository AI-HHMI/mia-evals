"""Skeleton-based instance-segmentation metrics: nERL, VOI, merge/split counts.

RECYCLED FROM BANIS, VERBATIM. Source: `metrics.py` of
...
`adapted_erl` and `evaluate_skeletons` are Zuzana Urbanova's additions (commit cb31913), adapted
in turn from `funlib.evaluate.expected_run_length`. Added here: this docstring; the `argparse`
import and the `__main__` block are dropped, since this is now a library. Nothing else changed.

**Do not "improve" these functions.** Same reasoning as `connected_components`: they are the
scoring half of the published NISB baseline, and reimplementing them would mean reporting against
a private approximation of the benchmark.

Needs `funlib.evaluate`, which is git-install-only and is why this module sits behind the
`instance` extra rather than in the core dependencies.
"""
